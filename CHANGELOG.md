...
- `identity-oracle`, `credit-oracle`, and `revocation-registry`: `initialize` now emits an `Initialized` event with topic `Symbol("Initialized")` (data still `admin: Address`) instead of the short-form `Init` symbol. Previously only `governance` emitted `Symbol("Initialized")`, so `docs/event-indexing.md` catalogued `Initialized` for all four contracts while only governance actually produced it — indexers filtering on `Initialized` silently missed identity-oracle, credit-oracle, and revocation-registry deployments. All four contracts now emit the identical topic. `docs/event-indexing.md` already documented `Symbol("Initialized")` and required no change (#664)
